...
if [[ "$FILE_PATH" == */docs/afeaturemerge/* ]]; then
    cat <<EOF
文件已保存：$FILE_PATH
请尝试将此文件同步到云端知识库，并告知用户结果：
- 若当前 session 有可用的知识库 MCP 工具（如钉钉、语雀、Confluence 等），执行同步：若本次任务尚未建立任务文件夹则先创建，再在文件夹下创建文档
- 若没有任何可用的知识库 MCP 工具，告知用户："文件已保存到本地 $FILE_PATH，未检测到云端知识库 MCP，未同步云端。如需同步，请配置对应的 MCP 工具。"
EOF
fi
